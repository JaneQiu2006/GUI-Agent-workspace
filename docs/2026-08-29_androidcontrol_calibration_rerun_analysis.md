# 2026-08-29 AndroidControl Calibration Rerun Analysis
...
- SCROLL 方向语义已对齐 AndroidControl：SCROLL step success 从 0 / 7 提升到 7 / 7。

主观察口径应使用 `gui_only`，即排除 `OPEN_APP` 和 `WAIT` 的普通 GUI 操作。新结果中：

- `gui_only` type accuracy 为 97.30%。
- `gui_only` step success rate 为 91.89%。
- `gui_only` trajectory success rate 为 80.00%。
- strict step success rate 为 74.51%，主要仍被 `OPEN_APP` 和 `WAIT` 拉低。
...
| Metric | Original Baseline | Prompt Rerun | Current Rerun |
| --- | ---: | ---: | ---: |
| num_steps | 51 | 51 | 51 |
...
| gui_only trajectory_success_rate | not reported | 20.00% | 80.00% |
...
| open_app only | 6 | 66.67% | 50.00% | 50.00% | 12.67 | 4.12s |
| wait only | 8 | 12.50% | 12.50% | 0.00% | 16.12 | 4.17s |
...
CLICK 已经从上一轮的 75.00% type accuracy / 70.83% step success 提升到 100.00% / 91.67%。这说明 malformed tap JSON 修复有效。当前 CLICK 的剩余失败是坐标偏差，不是 parser 问题：
...
OPEN_APP 从上一轮的 0.00% step success 提升到 50.00%。这来自 `open/openApp/launch` 归一化到 `OPEN_APP [...]`，但仍不建议把 OPEN_APP 混入普通 GUI 操作能力：
...
- WAIT only step success 为 12.50%。
...
当前最可信的结论是：Qwen3.8 在 AndroidControl mini 的普通静态 GUI 操作上已经达到较高命中率，`gui_only` step success 为 91.89%。最初 41.18% 的 strict step success 明显低估了能力，因为它混合了 thinking 截断、malformed JSON、SCROLL 方向语义、OPEN_APP 环境动作和 WAIT 静态噪声。
